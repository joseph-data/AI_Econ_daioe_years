...
with ui.navset_pill_list(id="tab", widths=(2, 10)):
    with ui.nav_panel("Occupation View", value="occupations"), ui.card():  # noqa: SIM117
    ...
    This panel presents the changes in employment over the years by occupation across the **SSYK 2012** levels. We consider also an overview by age and sex.
                """)
        ...
        with ui.card():
            ...
            with ui.layout_columns(col_widths=(4, 8)):
                ...
                with ui.card(full_screen=False):
                    ...
                    @render.download(
                        filename=lambda: (
                            f"ai_exposure_data.{download_extension(input.download_format() or 'csv')}"
                        ),
                        media_type=lambda: download_media_type(
                            input.download_format() or "csv",
                        ),
...
def q_base() -> pl.LazyFrame:
    ...
    ).select(pl.col(first_cols), pl.col(f"^(pctl_)?{metric}.*$"))
    return q.cache()
